...
class predatorPreyCustomEnv(gym.Env):
    def __init__(self, mode="MLP"):
        ...
        if mode=="CNN":
            self.observation_space = spaces.Box(low=0, high=255,
                                            shape=(120,160,1), dtype=np.uint8)#input image as observation
        elif self.mode=="MLP":
    ...
    def step(self,action):
        # reward=0
        # reward=0.03
        # reward=0.008*self.numPrey
        reward=0.01*self.numPrey
        over=False
        if len(self.preyList)==0 or self.frameIter>9000:
        ...
        if (collisionCheck[0]==1):#collision with a predator
            if(self.predatorList[0].isAvailable==True):
                ...
                reward-=0.5
                self.bounceBack(0,collisionCheck[1],1)#predator is occupied
    ...
    def getStateMLP(self):
        ...
        for prey in self.preyList:
            preyPos[idx]=prey.x/WIDTH
            preyPos[idx+1]=prey.y/HEIGHT
            idx+=2

        if self.numPrey>2:
            for prey in self.preyList:
                herdCenter[0]+=prey.rect.centerx
                herdCenter[1]+=prey.rect.centery
            herdCenter=herdCenter/self.numPrey
        else:
            herdCenter[0]=self.preyList[0].rect.centerx
            herdCenter[1]=self.preyList[0].rect.centery
        # herdPredDist=distance(herdCenter[0],herdCenter[1],self.predatorList[0].x,self.predatorList[0].y)#1
        ...
        # predatorDanger=[ 
        # (self.predatorList[0].rect.centerx > herdCenter[0])*min((1/max((abs(self.predatorList[0].rect.centerx-herdCenter[0])/WIDTH),0.01))/2,10)/10,  # predator right
        # (self.predatorList[0].rect.centerx < herdCenter[0])*min((1/max((abs(herdCenter[0]-self.predatorList[0].rect.centerx)/WIDTH),0.01))/2,10)/10,  # predator left
        # (self.predatorList[0].rect.centery < herdCenter[1])*min((1/max((abs(self.predatorList[0].rect.centery-herdCenter[1])/HEIGHT),0.01))/2,10)/10, # predator up
        # (self.predatorList[0].rect.centery > herdCenter[1])*min((1/max((abs(herdCenter[1]-self.predatorList[0].rect.centery)/HEIGHT),0.01))/2,10)/10  # predator down
        # ]
        agentDirection=[
        ...
        #4+4+4+20+2+1=35
        state=np.concatenate((boundaryDanger,predatorDanger,agentDirection,preyPos,predatorPos,np.array([self.numPrey])))
        # state=np.concatenate((boundaryDanger,predatorDanger,agentDirection,rlAgentPos,predatorPos))
        return state
    def update(self):
        WIN.fill(BLACK)
